...
import { FAMILY_ADVISORS } from "./init.mjs";
...
Your primary advisors for this project: ${FAMILY_ADVISORS[contract.project.family] ?? "all advisors"}
...
- For deeper doctrine, run \`${contract.runtime.command} coach --cwd . --json --intent "<topic>"\`
...
- **User starts designing a new system or feature** → suggest \`${contract.runtime.command} coach --cwd . --intent "<topic>"\` for deeper doctrine
- **User finishes building something** → suggest \`${contract.runtime.command} ship lite --cwd . --intent "<what was built>"\`
- **User is touching player-facing, economy, or multi-system code** → suggest \`${contract.runtime.command} ship full --cwd . --intent "<summary>"\`
- **Something is broken in production** → suggest \`${contract.runtime.command} hotfix --cwd . --json --env prod --intent "<what broke>"\`
- **User is wrapping up or switching tasks** → suggest \`${contract.runtime.command} handoff --cwd . --slug <slug> --summary "<summary>" --next "<next step>"\`
...
- Designing something new → \`${contract.runtime.command} coach\`
- Finished building → \`${contract.runtime.command} ship lite\`
- Touching player-facing or economy code → \`${contract.runtime.command} ship full\`
- Something broken → \`${contract.runtime.command} hotfix\`
- Wrapping up → \`${contract.runtime.command} handoff\`
